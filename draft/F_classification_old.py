...
class classification_model():
  ...
  def classification_decision_automatic(self
      ...
      classification['set_type'] = set_type
  ...
  def classification_decision(  self
    ...
    # extracting columns with probabilities
    prob = prob_f[self.y_labels]
    prob_f = prob_f.drop(self.y_labels, axis=1)
    ...
    # classification based on threshold (if threshold provided and target is binary)
    ...
    # sprawdzic 
    self.classification = pd.concat([self.classification.loc[(self.classification['if_automatic']!=0) | (self.classification['simulation_name']!=simulation_name) ,:], prob_f], ignore_index=True)
    # self.classification = pd.concat([ self.classification.loc[self.classification['if_automatic']==1,:], prob_f ], ignore_index = True)
  
  
  def scores_determining(self, y_true, y_pred, pos_label = 1):
    
    """
    calculation scores for models predictions 
    """
  ...
  def performance(  self
    ...
    # checking if simulation exists
    # if self.simulation_metainfo.shape != (0,0):
    #   if simulation_name not in list(self.simulation_metainfo.simulation_name):
    #     raise Exception('LUCAS: Simulation', simulation_name, 'name does not exists')
    
    # classification decision (not automatic)
    self.classification_decision(priori = priori, threshold = threshold, simulation_name = simulation_name)
    ...
    # getting data with classification decision
    cls = self.classification.loc[self.classification['simulation_name']==simulation_name, : ]
    
    # data grouping with classification decision
    group_structure = ['simulation_name', 'model_name', 'set_type', 'sample_type', 'sample_nr', 'sample_name',  'if_automatic' ]
    ...
    if confusion_matrix.shape[0] != 0:
        if len(self.confusion_matrix) != 0:
          self.confusion_matrix = pd.concat([self.confusion_matrix.loc[self.confusion_matrix['simulation_name'] != simulation_name, : ], confusion_matrix])
        else:
    ...
    self.simulation_metainfo.loc[self.simulation_metainfo['simulation_name']==simulation_name, 'priori']            = str(priori)
    self.simulation_metainfo.loc[self.simulation_metainfo['simulation_name']==simulation_name, 'threshold']         = str(threshold)
  ...
  def simulation(  self
    ...
    if cv_n is not None:
        ...
        for j in range(len(models_list)):
            ...
            for train_index, test_index in kf.split(x_train) :
                ...
                x_train_i, x_valid_i = x_train.iloc[train_index], x_train.iloc[test_index]
                y_train_i, y_valid_i = y_train.iloc[train_index], y_train.iloc[test_index]
                ...
                if cv_save_prob:
                    prob_train = pd.DataFrame(model_j.predict_proba(x_train_i), columns = [str(x) for x in model_j.classes_])
                    ...
                    prob_valid = pd.DataFrame(model_j.predict_proba(x_valid_i), columns = [str(x) for x in model_j.classes_])        
                    ...
                    if y_test is not None:
                        prob_test = pd.DataFrame(model_j.predict_proba(x_test), columns = [str(x) for x in model_j.classes_])  
    ...
    if hold_n is not None:
        ...
        for j in range(len(models_list)):
            ...
            for i in train_index_all.keys():
                ...
                valid_index = valid_index_all[i]
                
                # building sets based on split indicies
                x_train_i, x_valid_i = x_train.iloc[train_index], x_train.iloc[valid_index]
                y_train_i, y_valid_i = y_train.iloc[train_index], y_train.iloc[valid_index]
      ...
      for j in range(len(models_list)):
          ...
          for f, i in enumerate(by_unique): 
            ...
            if hold_save_prob:
                ...
                if y_test is not None:
                    prob_test = pd.DataFrame(model_j.predict_proba(x_test_i), columns = [str(x) for x in model_j.classes_])
                    print(prob_test.shape)
                    self.probabilities_determining(y_true = y_test
    ...
    if full is not None:
        ...
        for j in range(len(models_list)):
            ...
            if full_save_prob:
                    ...
                    prob_train = pd.DataFrame(model_j.predict_proba(x_train), columns = [str(x) for x in model_j.classes_])
        ...
        for f in range(fip_n):
            ...
            for column in x_train.columns:
                ...
                if fip_save_prob:
                    ...
                    prob_train = pd.DataFrame(model_fip.predict_proba(x_train_fip_perm), columns = [str(x) for x in model_fip.classes_])
                    self.probabilities_determining(y_true = y_train_fip
                                                  , prob = prob_train, simulation_name = simulation_name, model_name = model_fip_name, sample_type = sample_type, sample_nr = f, set_type = 'train')
                    
                    prob_valid = pd.DataFrame(model_fip.predict_proba(x_valid_fip), columns = [str(x) for x in model_fip.classes_])             
                    self.probabilities_determining(y_true = y_valid_fip
                                                  , prob = prob_valid, simulation_name = simulation_name, model_name = model_fip_name, sample_type = sample_type, sample_nr = f, set_type = 'valid' )
                    
                    if y_test is not None:
                        prob_test = pd.DataFrame(model_fip.predict_proba(x_test), columns = [str(x) for x in model_fip.classes_])  
                        self.probabilities_determining(y_true = y_test
                                                      , prob = prob_test, simulation_name = simulation_name, model_name = model_fip_name, sample_type = sample_type, sample_nr = f, set_type = 'test')
                
                # classification calculation and saving (automatic based on default setting in 'proba' function)
                if fip_save_class:
                    
                    self.classification_decision_automatic( x = x_train_fip_perm, y_true = y_train_fip
                                                          , model = model_fip, simulation_name = simulation_name, model_name = model_fip_name, sample_type = sample_type, sample_nr = f, set_type = 'train')
                    
                    self.classification_decision_automatic(  x = x_valid_fip, y_true = y_valid_fip
                                                           , model = model_fip, simulation_name = simulation_name, model_name = model_fip_name, sample_type = sample_type, sample_nr = f, set_type = 'valid')
                    if y_test is not None:
                        self.classification_decision_automatic(  x = x_test, y_true = y_test
                                                               , model = model_fip, simulation_name = simulation_name, model_name = model_fip_name, sample_type = sample_type, sample_nr = f, set_type = 'test')
    ...
    if correlation:
      ...
      if self.cat_vars is not None and self.num_vars is not None:
        ...
        correlation_mix_ratio = self.correlation_ratio_matrix(data = x_train_b, vars_cat = self.cat_vars, vars_num = self.num_vars, round = 2)
  ...
  def scores_cf_agg_full(self, simulation_params = [], if_automatic = 0, pos_label = '1'):
    ...
    for params in simulation_params:
        data = pd.concat([data, data_f.loc[(data_f['simulation_name'] == params[0]) & (data_f['model_name'] == params[1]), : ]], axis = 0)
    ...
    group_structure = ['if_automatic', 'model_name', 'sample_nr', 'sample_name', 'sample_type', 'set_type', 'simulation_name']
    ...
    train_group = pd.DataFrame([[if_automatic, model_name_agg, 0, '_', 'full', 'train', simulation_name_agg ]], columns = group_structure )
    test_group  = pd.DataFrame([[if_automatic, model_name_agg, 0, '_', 'full', 'test',  simulation_name_agg ]], columns = group_structure )
  ...
  def fast_scores_plot( self
    ...
    display( self.plot_scores(filter = {'if_automatic':[0], 'sample_type':[sample_type], 'set_type' : ['train'], 'simulation_name':[simulation_name]}, x_var = 'sample_nr', y_var='score_value', fill_var = 'model_name', facet = '.~score_name')
            ,self.plot_scores(filter = {'if_automatic':[0], 'sample_type':[sample_type], 'set_type' : ['test'],  'simulation_name':[simulation_name]}, x_var = 'sample_nr', y_var='score_value',  fill_var = 'model_name', facet = '.~score_name'))
  ...
  def fast_conf_matrix_scores(self, simulation_name):
    ...
    """
    Printing confusion matrix, scores for sample_type = 'full' but for all models. Additionaly information about y_counts and threshold are printted.
    """
    ...
    for i in models_list:
      if len(self.confusion_matrix.loc[(self.confusion_matrix['sample_type']=='full') & (self.confusion_matrix['model_name']==i),:] ) != 0: # protection for 'fip'
    ...
    for i in scores_names_list:
      ...
      scores_i = self.scores.loc[(self.scores['if_automatic']==0) & (self.scores['simulation_name']==simulation_name) & (self.scores['sample_type']=='full') & (self.scores['score_name']==i),['model_name',	'set_type', 'score_value']] 
    ...
    #(5) threshold
    threshold = self.threshold.get(simulation_name, {'no_threshold':'NaN'})
    threshold = pd.DataFrame(list(threshold.values()), index = list(threshold.keys()))
    
    
    
    # displaying all elements (1-5)
    
    return([ self.sbs( [y_train_count, y_test_count, threshold]       ,['target train','target test', 'thresholds'] )
            ,self.sbs( cf_train_list                                  ,['Confusion matrix  TRAIN ,   Model:  '   + x for x in models_list] )
            ,self.sbs( cf_test_list                                   ,['Confusion matrix  TEST,    Model:  '  + x for x in models_list] )
  ...
  def performance_binary_multi_thr(self, thresholds_list = pd.DataFrame({'1':np.arange(0.5,  1, 0.05)}), model_name = None, simulation_name = None, pos_label = '1' ):
    
    thresholds_list = pd.DataFrame({'1':np.arange(0.5,  1, 0.05)})
    thresholds_list['0'] = 1 - thresholds_list['1']
    
    cf     = pd.DataFrame()
    scores = pd.DataFrame()
    
    for i in range(len(thresholds_list)):
        threshold = {'0':thresholds_list.loc[i,'0'], '1':thresholds_list.loc[i,'1']}
        cut_off = thresholds_list.loc[i,'1']
        
        # performance is overwritten in each iteration
        self.performance(simulation_name=simulation_name, threshold=threshold, priori =  None, pos_label = pos_label)
        
        cf_i = self.confusion_matrix.loc[(self.confusion_matrix['simulation_name']  == simulation_name) & 
                                        (self.confusion_matrix['model_name']      == model_name)	 & 
                                        (self.confusion_matrix['sample_type']     == 'full') & 
                                        (self.confusion_matrix['sample_nr']       == 0)  & 
                                        (self.confusion_matrix['if_automatic']    == 0) 
                                        , ['model_name','set_type', 0,1,2,3]]
        cf_i['cut_off'] =  cut_off
        scores_i = self.scores.loc[(self.scores['simulation_name'] == simulation_name) & 
                              (self.scores['model_name']      == model_name)	 & 
                              (self.scores['sample_type']     == 'full') & 
                              (self.scores['sample_nr']       == 0)  & 
                              (self.scores['if_automatic']    == 0) 
                            , ['model_name','set_type', 'score_name', 'score_value']]
        scores_i['cut_off'] = cut_off
        
        cf     = pd.concat([cf, cf_i])
        scores = pd.concat([scores, scores_i])
      
    scores = pd.pivot_table(scores, index=['model_name', 'set_type','cut_off'], values = 'score_value')
...
# Praca zdalna mi doskwiera, ciągłe siedzenie w domu coraz bardziej uwiera.  Jednak nie trace nadziej że to się wkrótce zmieni. I znowu wszyscy razem pod jednym dachem będziemy wspólnie sprintować. 
