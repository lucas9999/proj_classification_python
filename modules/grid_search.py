...
class Objective(object):
    ...
    def __call__(self, trial):
      ...
      aps = average_precision_score(y_true=self.y_test, y_score=prob[str(self.pos_label)] , pos_label=str(self.pos_label) )
      
      skm.recall_score(y_true = self.y_test.astype(int), y_pred = classification.astype(int), average = 'binary', pos_label = self.pos_label)
      
      # recall
      try:
        if len(self.y_labels) < 3:
          recall  = skm.recall_score(y_true = self.y_test.astype(int), y_pred = classification.astype(int), average = 'binary', pos_label = self.pos_label)
        else:
          recall = skm.recall_score(y_true = self.y_test.astype(str), y_pred = classification.astype(str), average = 'weighted')
      except:
          recall = np.nan
      ...
      try:
        if len(self.y_labels) < 3:
          precision  = skm.precision_score(y_true = self.y_test.astype(int), y_pred = classification.astype(int), average = 'binary', pos_label = self.pos_label)
        else:
          precision = skm.precision_score(y_true = self.y_test.astype(str), y_pred = classification.astype(str), average = 'weighted')
      except:
          precision = np.nan
      ...
      try:
        if len(self.y_labels) < 3:
          f1 = skm.f1_score(y_true = self.y_test, y_pred = classification, average = 'binary', pos_label = str(self.pos_label))
        else:
          f1 = skm.f1_score(y_true = self.y_test, y_pred = classification, average = 'weighted')
      except:
        f1 = np.nan
      
      scores = pd.DataFrame([[balanced_accuracy, accuracy, recall, precision, f1, aps]], columns = ['balanced_accuracy', 'accuracy', 'recall', 'precision', 'f1', 'aps'])
...
def grid_optuna_scores_plot(data, fig_w = 10, fig_h = 10):
  ...
  print(ggplot(data = data) + geom_line(aes(x = 'index', y = 'value', color = 'threshold')) + facet_grid('variable~method', scales  = 'free_y') )
